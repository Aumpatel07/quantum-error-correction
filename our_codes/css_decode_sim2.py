...
from Restriction_mapping import mapping_error_on_qubits_of_two_edge_augmented_surface_codes_to_error_in_colour_codes,mapping_syndrome_from_colour_code_to_two_edge_augmented_surface_codes,mapping_syndrome_from_edge_augmented_surface_codes_to_normal_surface_codes,mapping_error_from_normal_surface_codes_to_edge_augmented 
...
class css_decode_sim2():
    ...
    def __init__(self,hx_surface_codes=None,hx_edge_augmented_surface_codes=None,hx=None, hz=None , **input_dict):
        ...
        self.hx_surface_codes = hx_surface_codes.astype(int)
        self.hx_edge_augmented_surface_codes = hx_edge_augmented_surface_codes.astype(int)
    ...
    def _single_run(self):
        ...
        if self.channel_update is None:
            ...
            syn1,syn2 = mapping_syndrome_from_colour_code_to_two_edge_augmented_surface_codes(self.hx,self.hx_edge_augmented_surface_codes,synd_z)
            syn_on_surface_code1,estimated_error1,qubits_with_half_prob1 = mapping_syndrome_from_edge_augmented_surface_codes_to_normal_surface_codes(self.hx_edge_augmented_surface_codes,self.hx_surface_codes,syn1)
            syn_on_surface_code2,estimated_error2,qubits_with_half_prob2 = mapping_syndrome_from_edge_augmented_surface_codes_to_normal_surface_codes(self.hx_edge_augmented_surface_codes,self.hx_surface_codes,syn2)

            new_error_channel1 = self.error_rate  * (self.channel_probs_z[:self.hx_surface_codes.shape[1]] + self.channel_probs_y[:self.hx_surface_codes.shape[1]])
            new_error_channel2 = self.error_rate  * (self.channel_probs_z[:self.hx_surface_codes.shape[1]] + self.channel_probs_y[:self.hx_surface_codes.shape[1]])

            new_error_channel1[ qubits_with_half_prob1 == 1] = 0.5
            new_error_channel2[ qubits_with_half_prob2 == 1] = 0.5
            
            self.bpd_z1.update_channel_probs(new_error_channel1)
            self.bpd_z2.update_channel_probs(new_error_channel2)

            self.bpd_z1.decode(syn_on_surface_code1)
            self.bpd_z2.decode(syn_on_surface_code2)
            


            edge_augmented_code_error1 = mapping_error_from_normal_surface_codes_to_edge_augmented(self.hx_edge_augmented_surface_codes,self.hx_surface_codes,self.bpd_z1.osdw_decoding)
            edge_augmented_code_error2 = mapping_error_from_normal_surface_codes_to_edge_augmented(self.hx_edge_augmented_surface_codes,self.hx_surface_codes,self.bpd_z2.osdw_decoding)

            error1 = (edge_augmented_code_error1 + estimated_error1) % 2
            error2 = (edge_augmented_code_error2 + estimated_error2) % 2

            # print("osdw")
            self.zosdw_decoding = mapping_error_on_qubits_of_two_edge_augmented_surface_codes_to_error_in_colour_codes(self.hx,self.hx_edge_augmented_surface_codes,error1,error2)
            # print("osd0")
            self.zosd0_decoding = self.zosdw_decoding
            # print("bp")
            self.zbp_decoding = self.zosdw_decoding
            # self.zbp_decoding = mapping_error_on_qubits_of_two_edge_augmented_surface_codes_to_error_in_colour_codes(self.hx,self.hx_surface_codes,self.bpd_z1.bp_decoding,self.bpd_z2.bp_decoding)
            # print("done for this run")
            # print("original syn",synd_z)
            # print("mapped synd", syn1,syn2)
            # print("osdw errors",self.bpd_z1.osdw_decoding,self.bpd_z2.osdw_decoding)
            # print("mapped error",self.zosdw_decoding )

            # decode x
            synd_x = self.hz@self.error_x % 2
            syn3, syn4 = mapping_syndrome_from_colour_code_to_two_edge_augmented_surface_codes(self.hz,self.hx_edge_augmented_surface_codes,synd_x)
            syn_on_surface_code3,estimated_error3,qubits_with_half_prob3 = mapping_syndrome_from_edge_augmented_surface_codes_to_normal_surface_codes(self.hx_edge_augmented_surface_codes,self.hx_surface_codes,syn3)
            syn_on_surface_code4,estimated_error4,qubits_with_half_prob4 = mapping_syndrome_from_edge_augmented_surface_codes_to_normal_surface_codes(self.hx_edge_augmented_surface_codes,self.hx_surface_codes,syn4)

            new_error_channel3 = self.error_rate  * (self.channel_probs_x[:self.hx_surface_codes.shape[1]] + self.channel_probs_y[:self.hx_surface_codes.shape[1]])
            new_error_channel4 = self.error_rate  * (self.channel_probs_x[:self.hx_surface_codes.shape[1]] + self.channel_probs_y[:self.hx_surface_codes.shape[1]])

            new_error_channel3[ qubits_with_half_prob3 == 1] = 0.5
            new_error_channel4[ qubits_with_half_prob4 == 1] = 0.5

            self.bpd_x1.update_channel_probs(new_error_channel3)
            self.bpd_x2.update_channel_probs(new_error_channel4)

            self.bpd_x1.decode(syn_on_surface_code3)
            self.bpd_x2.decode(syn_on_surface_code4)


            edge_augmented_code_error3 = mapping_error_from_normal_surface_codes_to_edge_augmented(self.hx_edge_augmented_surface_codes,self.hx_surface_codes,self.bpd_x1.osdw_decoding)
            edge_augmented_code_error4 = mapping_error_from_normal_surface_codes_to_edge_augmented(self.hx_edge_augmented_surface_codes,self.hx_surface_codes,self.bpd_x2.osdw_decoding)

            error3 = (edge_augmented_code_error3 + estimated_error3) % 2
            error4 = (edge_augmented_code_error4 + estimated_error4) % 2

            self.xosdw_decoding = mapping_error_on_qubits_of_two_edge_augmented_surface_codes_to_error_in_colour_codes(self.hz,self.hx_edge_augmented_surface_codes,error3,error4)
            self.xosd0_decoding = self.xosdw_decoding
            self.xbp_decoding = self.xosdw_decoding
    ...
    def _encoded_error_rates(self):
        ...
        residual_x = (self.error_x+self.xosdw_decoding) % 2
        residual_z = (self.error_z+self.zosdw_decoding) % 2
        ...
        # calculate the residual error
        residual_x = (self.error_x+self.xosd0_decoding) % 2
        residual_z = (self.error_z+self.xosd0_decoding) % 2
        ...
        if self.bpd_z1.converge:
            self.bp_converge_count_z+=1
        if self.bpd_x1.converge:
            self.bp_converge_count_x+=1

        if self.bpd_z1.converge and self.bpd_x1.converge:
            # calculate the residual error
            residual_x = (self.error_x+self.xbp_decoding) % 2
            residual_z = (self.error_z+self.zbp_decoding) % 2
    ...
    def _decoder_setup(self):
        ...
        channel_probs_z = self.channel_probs_z[:self.hx_surface_codes.shape[1]]
        channel_probs_x = self.channel_probs_x[:self.hx_surface_codes.shape[1]]
        channel_probs_y = self.channel_probs_y[:self.hx_surface_codes.shape[1]]

        # decoder for Z errors
        self.bpd_z1 = bposd_decoder(
            self.hx_surface_codes,
            channel_probs=channel_probs_z+channel_probs_y,
        ...
        self.bpd_x1 = bposd_decoder(
            self.hx_surface_codes,
            channel_probs=channel_probs_x+channel_probs_y,
            max_iter=self.max_iter,
            bp_method=self.bp_method,
            ms_scaling_factor=self.ms_scaling_factor,
            osd_method=self.osd_method,
